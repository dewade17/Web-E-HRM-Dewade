// app/api/admin/kategori-izin-jam/route.js
...
export async function GET(req) {
  ...
  try {
    ...
    const [total, data] = await Promise.all([
      db.kategoriIzinJam.count({ where }),
      db.kategoriIzinJam.findMany({
        where,
    ...
    console.error('GET /admin/kategori-izin-jam error:', err);
...
export async function POST(req) {
  ...
  try {
    ...
    // Cek duplikasi sederhana berdasarkan nama_kategori
    const existing = await db.kategoriIzinJam.findFirst({
      where: { nama_kategori },
    ...
    const created = await db.kategoriIzinJam.create({
      data: { nama_kategori },
      select: {
        id_kategori_izin_jam: true,
        nama_kategori: true,
    ...
    return NextResponse.json({ message: 'Kategori izin jam dibuat.', data: created }, { status: 201 });
  } catch (err) {
    if (err?.code === 'P2002') {
      return NextResponse.json({ message: 'Kategori izin jam sudah terdaftar.' }, { status: 409 });
    }
    console.error('POST /admin/kategori-izin-jam error:', err);
    return NextResponse.json({ message: 'Server error.' }, { status: 500 });
  }
}
