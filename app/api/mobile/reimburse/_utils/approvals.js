...
function collectStructuredApprovals(body) {
  ...
  const items = {};
  const pattern = /^(?:approvals?|approval)(?:\[(\d+)\])?[._-]?(level|approver_user_id|approver_role|note|id|id_approval_reimburse)$/i;
...
function sanitizeApprovalRecord(raw, index) {
  ...
  const idRaw = raw.id_approval_reimburse ?? raw.id ?? raw.approval_id;
...
export async function syncApprovalRecords(tx, reimburseId, desiredApprovals) {
  ...
  const existing = await tx.approvalReimburse.findMany({
    where: { id_reimburse: reimburseId, deleted_at: null },
  });

  const existingById = new Map();
  for (const item of existing) existingById.set(item.id_approval_reimburse, item);
    ...
    if (incomingId && existingById.has(incomingId)) {
      ...
      if (changed) {
        await tx.approvalReimburse.update({
          where: { id_approval_reimburse: incomingId },
          data: {
  ...
  const deletions = existing.filter((item) => !desiredIds.has(item.id_approval_reimburse)).map((item) => item.id_approval_reimburse);

  if (deletions.length) {
    await tx.approvalReimburse.deleteMany({
      where: { id_approval_reimburse: { in: deletions } },
    });
  }

  if (creations.length) {
    await tx.approvalReimburse.createMany({ data: creations });
  }
}
