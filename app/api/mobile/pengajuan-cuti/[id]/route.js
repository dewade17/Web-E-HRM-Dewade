 ...
 * @param {Array<{tanggal_cuti: Date}>} tanggalList
 */
...
// -----------------------------------------------------------------
// ▼▼▼ LOGIKA SINKRONISASI SHIFT BERDASARKAN TANGGAL CUTI LIST ▼▼▼
// -----------------------------------------------------------------
...
async function syncShiftLiburForApprovedLeave(tx, { userId, tanggalList, returnDate, returnShift }) {
  ...
  // Penyesuaian shift HARI MASUK KERJA (opsional)
...
/**
 * PATCH/PUT /api/mobile/pengajuan-cuti/approvals/[id]
...
async function handleDecision(req, { params }) {
  ...
  try {
    ...
    // Notifikasi penyesuaian shift saat cuti (rename periode_mulai -> periode_cuti)
