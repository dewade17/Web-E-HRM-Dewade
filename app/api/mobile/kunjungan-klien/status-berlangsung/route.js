...
export async function GET(req) {
  ...
  try {
    ...
    const where = {
      id_user: actorId,
      status_kunjungan: 'berlangsung', // Filter utama
      deleted_at: null,
    };
    ...
    if (tanggalParam) {
      ...
      // Buat rentang dari awal hingga akhir hari untuk mencakup semua data pada tanggal tersebut
    ...
    const [total, items] = await Promise.all([
      ...
      db.kunjungan.findMany({
        where,
        orderBy: { jam_checkin: 'desc' }, // Urutkan berdasarkan waktu check-in terbaru
    ...
    console.error('GET /status-berlangsung error:', err);
