...
export async function GET(req) {
  ...
  try {
    ...
    const kategoriId = (searchParams.get('id_master_data_kunjungan') || searchParams.get('kategoriId') || '').trim();
    const tanggalParam = (searchParams.get('tanggal') || '').trim();

    const filters = [{ id_user: actorId }, { deleted_at: null }];

    if (kategoriId) {
      filters.push({ id_master_data_kunjungan: kategoriId });
    }
...
export async function POST(req) {
  ...
  try {
    ...
    if (isNullLike(rawKategori)) {
      return NextResponse.json({ message: "Field 'id_master_data_kunjungan' wajib diisi." }, { status: 400 });
    }
    const kategoriId = String(rawKategori).trim();
    ...
    const tanggal = new Date(String(rawTanggal));
    if (Number.isNaN(tanggal.getTime())) {
    ...
    const data = {
      id_user: actorId,
      id_master_data_kunjungan: kategoriId,
      tanggal,
    };
    ...
    if (err?.code === 'P2002') {
      return NextResponse.json({ message: 'Data kunjungan sudah ada.' }, { status: 409 });
    }
    if (err?.code === 'P2003') {
