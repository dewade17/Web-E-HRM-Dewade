...
function collectStructuredApprovals(body) {
  ...
  const items = {};
  const pattern = /^(?:approvals?|approval)(?:\[(\d+)\])?[._-]?(level|approver_user_id|approver_role|note|id|id_approval_pocket_money)$/i;
...
function sanitizeApprovalRecord(raw, index) {
  ...
  const idRaw = raw.id_approval_pocket_money ?? raw.id ?? raw.approval_id;
...
export async function syncApprovalRecords(tx, reimburseId, desiredApprovals) {
  ...
  const existing = await tx.approvalPocketMoney.findMany({
    where: { id_pocket_money: reimburseId, deleted_at: null },
  });

  const existingById = new Map();
  for (const item of existing) existingById.set(item.id_approval_pocket_money, item);
    ...
    if (incomingId && existingById.has(incomingId)) {
      ...
      if (changed) {
        await tx.approvalPocketMoney.update({
          where: { id_approval_pocket_money: incomingId },
          data: {
  ...
  const deletions = existing.filter((item) => !desiredIds.has(item.id_approval_pocket_money)).map((item) => item.id_approval_pocket_money);

  if (deletions.length) {
    await tx.approvalPocketMoney.deleteMany({
      where: { id_approval_pocket_money: { in: deletions } },
    });
  }

  if (creations.length) {
    await tx.approvalPocketMoney.createMany({ data: creations });
  }
}
