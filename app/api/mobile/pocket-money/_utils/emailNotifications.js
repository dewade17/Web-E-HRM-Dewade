...
function formatItems(items) {
  ...
  return items
    .map((item) => {
      const name = item?.nama_item_pocket_money || item?.nama_item || item?.nama || 'Item';
...
export async function sendPocketMoneyEmailNotifications(req, pocketMoney) {
  ...
  const jabatan = pocketMoney.user?.jabatan?.nama_jabatan || '-';
  const kategori = pocketMoney.kategori_keperluan?.nama_keperluan || '-';
  const tanggalLabel = formatDateDisplay(pocketMoney.tanggal);
  const totalLabel = formatCurrency(pocketMoney.total_pengeluaran);
  const metodeLabel = pocketMoney.metode_pembayaran || '-';
  const keterangan = stripUserIds(pocketMoney.keterangan);
  const itemsLabel = formatItems(pocketMoney.items);
  ...
  const approverEmails = uniqueEmails((pocketMoney.approvals || []).map((a) => a?.approver?.email));
  const ops = [];
  ...
  if (approverEmails.length) {
    ops.push(
      sendBatch({
        from,
        to: approverEmails,
        subject: `[E-HRM] Permintaan Persetujuan Pocket Money (${pemohonName})`,
        html: makeEmailHtml({
          title: 'Permintaan Persetujuan Pocket Money',
          introLines: [`Anda dipilih sebagai approver untuk pengajuan pocket money dari ${pemohonName}.`],
          fields,
          buttonUrl: url,
  ...
  if (pemohonEmail) {
    ops.push(
      sendBatch({
        from,
        to: [pemohonEmail],
        subject: `[E-HRM] Pengajuan Pocket Money Terkirim (${tanggalLabel})`,
        html: makeEmailHtml({
          title: 'Pengajuan Pocket Money Berhasil Dikirim',
          introLines: ['Pengajuan pocket money Anda telah berhasil dibuat dan dikirim untuk diproses.'],
          fields,
          buttonUrl: url,
