...
export async function GET(req, { params }) {
  ...
  try {
    ...
    console.error('GET /story-planner/[id] error:', err?.code || err);
...
export async function PUT(req, { params }) {
  ...
  try {
    ...
    return NextResponse.json({ message: 'Story planner diperbarui.', data: updated });
  } catch (err) {
    if (err?.code === 'P2025') {
      return NextResponse.json({ message: 'Story planner tidak ditemukan' }, { status: 404 });
    }
    console.error('PUT /story-planner/[id] error:', err?.code || err);
    return NextResponse.json({ message: 'Server error' }, { status: 500 });
  }
}
...
export async function DELETE(req, { params }) {
  ...
  try {
    ...
    if (existing.deleted_at) {
      return NextResponse.json({ message: 'Story planner sudah dihapus.' });
    }
    ...
    console.error('DELETE /story-planner/[id] error:', err?.code || err);
    return NextResponse.json({ message: 'Server error' }, { status: 500 });
  }
}
