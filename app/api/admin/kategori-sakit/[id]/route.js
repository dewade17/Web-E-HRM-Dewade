// app/api/admin/kategori-sakit/[id]/route.js
...
export async function GET(req, { params }) {
  ...
  try {
    ...
    const data = await db.kategoriSakit.findUnique({
      where: { id_kategori_sakit: id },
      select: {
    ...
    console.error('GET /admin/kategori-sakit/[id] error:', err);
...
export async function PUT(req, { params }) {
  ...
  try {
    ...
    const updated = await db.kategoriSakit.update({
      where: { id_kategori_sakit: params.id },
      data: payload,
      select: {
        id_kategori_sakit: true,
        nama_kategori: true,
    ...
    return NextResponse.json({ message: 'Kategori sakit diperbarui.', data: updated });
  } catch (err) {
    if (err?.code === 'P2002') {
    ...
    console.error('PUT /admin/kategori-sakit/[id] error:', err);
    return NextResponse.json({ message: 'Server error.' }, { status: 500 });
  }
}
...
export async function DELETE(req, { params }) {
  ...
  try {
    ...
    if (isHardDelete) {
      await db.kategoriSakit.delete({
        where: { id_kategori_sakit: params.id },
      });
      return NextResponse.json({ message: 'Kategori sakit dihapus secara permanen (hard delete).' });
    }

    await db.kategoriSakit.update({
      where: { id_kategori_sakit: params.id },
      data: { deleted_at: new Date() },
    });
    return NextResponse.json({ message: 'Kategori sakit dihapus (soft delete).' });
  } catch (err) {
    if (err?.code === 'P2025') {
      return NextResponse.json({ message: 'Kategori sakit tidak ditemukan.' }, { status: 404 });
    }
    console.error('DELETE /admin/kategori-sakit/[id] error:', err);
    return NextResponse.json({ message: 'Server error.' }, { status: 500 });
  }
}
