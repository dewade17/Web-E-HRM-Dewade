...
export async function GET(req) {
  ...
  try {
    ...
    const where = {
      ...
      ...(search
        ? {
            OR: [{ user: { nama_pengguna: { contains: search } } }, { user: { email: { contains: search } } }, { user: { nomor_induk_karyawan: { contains: search } } }],
          }
        : {}),
    };
...
export async function POST(req) {
  ...
  try {
    ...
    if (!Number.isInteger(kouta) || kouta < 0) {
      return NextResponse.json({ message: "Field 'kouta_cuti' harus berupa bilangan bulat >= 0." }, { status: 400 });
    }
