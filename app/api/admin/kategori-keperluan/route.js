...
const ALLOWED_ORDER_BY = new Set(['created_at', 'updated_at', 'nama_keperluan']);
...
export async function GET(req) {
  ...
  try {
    ...
    const [total, data] = await Promise.all([
      db.kategoriKeperluan.count({ where }),
      db.kategoriKeperluan.findMany({
        where,
    ...
    console.error('GET /admin/kategori-keperluan error:', err);
...
export async function POST(req) {
  ...
  try {
    ...
    const rawNama = body.nama_keperluan;

    if (rawNama === undefined || String(rawNama).trim() === '') {
      return NextResponse.json({ message: "Field 'nama_keperluan' wajib diisi." }, { status: 400 });
    }

    const nama_keperluan = String(rawNama).trim();

    const created = await db.kategoriKeperluan.create({
      data: { nama_keperluan },
      select: {
        id_kategori_keperluan: true,
        nama_keperluan: true,
    ...
    return NextResponse.json({ message: 'Kategori keperluan dibuat.', data: created }, { status: 201 });
  } catch (err) {
    if (err?.code === 'P2002') {
      return NextResponse.json({ message: 'Kategori keperluan sudah terdaftar.' }, { status: 409 });
    }
    console.error('POST /admin/kategori-keperluan error:', err);
    return NextResponse.json({ message: 'Server error.' }, { status: 500 });
  }
}
